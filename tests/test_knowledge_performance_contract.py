...
class KnowledgePerformanceContractTests(unittest.TestCase):
    ...
    def test_harness_uses_real_local_storage_paths_and_safety_sentinel(self) -> None:
        ...
        self.assertIn("KnowledgeStore.new()", text)
        self.assertIn("KnowledgeImportTransaction.new()", text)
        self.assertIn("KnowledgeSourceRegistry.new()", text)
        self.assertIn("MemoryStore.new()", text)
