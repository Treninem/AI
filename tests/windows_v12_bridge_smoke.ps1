...
try {
    ...
    $v13 = Start-Process -FilePath $currentInstallerPath -ArgumentList @('/VERYSILENT','/SUPPRESSMSGBOXES','/NORESTART',"/DIR=$installDir") -PassThru -Wait
    if ($v13.ExitCode -notin @(0,3010)) { throw "Current bridge installer exited with $($v13.ExitCode)" }
    ...
    if ($bridgeText -notmatch 'current=1\.3\.0\.0') { throw "Bridge marker did not record V1.3 target version: $bridgeText" }
    ...
    Write-Host 'AURORA_WINDOWS_V12_TO_V13_BRIDGE_OK' -ForegroundColor Green
} finally {
