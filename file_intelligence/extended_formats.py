...
def _safe_member(name: str) -> str:
    normalized = unquote(str(name)).replace("\\", "/")
...
def _bounded_zip_read(zf: zipfile.ZipFile, name: str) -> bytes:
    info = zf.getinfo(_safe_member(name))
    if info.file_size > MAX_EMBEDDED_TEXT_BYTES:
        raise ValueError("EPUB member exceeds bounded read limit")
    with zf.open(info) as member:
        raw = member.read(MAX_EMBEDDED_TEXT_BYTES + 1)
...
def _epub_nav(zf: zipfile.ZipFile, opf_dir: PurePosixPath, manifest: dict[str, dict[str, str]]) -> list[dict[str, str]]:
    ...
    if nav_item:
        ...
        try:
            raw = _bounded_zip_read(zf, nav_path)
    ...
    try:
        root = ET.fromstring(_bounded_zip_read(zf, ncx_path))
    except Exception:
        return []
...
def analyze_epub(path: Path, max_chars: int = 160000) -> tuple[str, dict[str, Any], list[str]]:
    warnings: list[str] = []
    ...
    with zipfile.ZipFile(path) as zf:
        ...
        try:
            container = ET.fromstring(_bounded_zip_read(zf, "META-INF/container.xml"))
        ...
        try:
            opf = ET.fromstring(_bounded_zip_read(zf, opf_path))
        except KeyError as exc:
        ...
        for order, item_id in enumerate(spine_ids, start=1):
            if remaining <= 0:
                warnings.append("EPUB text reached the requested character budget.")
                break
            ...
            try:
                raw = _bounded_zip_read(zf, chapter_path)
            except KeyError:
        ...
        return "\n".join(parts).strip()[:max(0, max_chars)], metadata, warnings


def analyze_rar(path: Path, max_chars: int = 160000) -> tuple[str, dict[str, Any], list[str]]:
    import rarfile

    warnings: list[str] = []
    backend = "stored-entry-reader"
    ...
    with rarfile.RarFile(str(path)) as rf:
        ...
        for info in infos:
            if extracted_text_entries >= MAX_ARCHIVE_TEXT_ENTRIES or remaining <= 0:
                break
            ...
            if info.compress_type != rarfile.RAR_M0:
                warnings.append(f"Compressed RAR text requires a separately bounded local extractor: {name}")
                continue
            try:
                with rf.open(info) as member:
                    data = member.read(min(MAX_EMBEDDED_TEXT_BYTES, remaining * 4) + 1)
                if len(data) > MAX_EMBEDDED_TEXT_BYTES:
                    raise ValueError("RAR member exceeds bounded read limit")
            except Exception as exc:
    ...
    return content[:max(0, max_chars)], metadata, warnings
