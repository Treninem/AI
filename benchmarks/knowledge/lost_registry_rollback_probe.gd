...
func _run() -> void:
	if OS.get_environment("AURORA_KNOWLEDGE_BENCH_ALLOW_DESTRUCTIVE") != "1":
		_emit({"ok": false, "error": "benchmark destructive sentinel missing"}, 90); return
	_reset_state()
	...
	if initial_file == null:
		_emit({"ok": false, "error": "cannot create stable source"}, 2); return
	initial_file.store_string(JSON.stringify({"fact": STABLE_MARKER, "description": "valid state before registry loss"})); initial_file.close()
	...
	if not bool(initial.get("ok", false)):
		_emit({"ok": false, "error": "initial import failed", "initial": initial}, 3); return
	...
	if broken == null:
		_emit({"ok": false, "error": "cannot create malformed replacement"}, 4); return
	broken.store_string('{"fact":"%s","broken":[1,2,' % PARTIAL_MARKER); broken.close()
	...
	var ok := bool(initial.get("ok", false)) and not stable_before.is_empty() and not registry_before.is_empty() and failed_as_expected and conservative_snapshot and preserved
	_emit({"ok": ok, "initial_import_ok": bool(initial.get("ok", false)), "stable_before": not stable_before.is_empty(), "registry_before": not registry_before.is_empty(), "registry_deleted_before_failure": true, "failed_as_expected": failed_as_expected, "transaction": failed.get("transaction", ""), "source_snapshot": failed.get("source_snapshot", ""), "conservative_snapshot": conservative_snapshot, "stable_after": not stable_after.is_empty(), "partial_after": not partial_after.is_empty(), "chunks_before": before_chunks, "chunks_after": after_chunks, "previous_valid_state_preserved": preserved, "expected_contract": "registry loss must disable first-import fast path so failed import cannot erase valid source rows"}, 0 if ok else 5)

func _reset_state() -> void:
	for path in ["user://knowledge/knowledge.jsonl", "user://knowledge/structured.jsonl", "user://knowledge/sources.json", "user://knowledge/.knowledge_source.txn.jsonl", "user://knowledge/.structured_source.txn.jsonl", "user://knowledge/.sources.json.txn", SOURCE]:
		if FileAccess.file_exists(path): DirAccess.remove_absolute(ProjectSettings.globalize_path(path))
	KnowledgeSourceRegistryScript.invalidate_runtime_cache()

func _emit(result: Dictionary, code: int) -> void:
	print(RESULT_PREFIX + JSON.stringify(result)); quit(code)
