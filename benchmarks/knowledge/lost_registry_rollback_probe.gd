...
func _run() -> void:
	...
	# Simulate recovery from a lost registry while valid Knowledge JSONL rows remain.
	...
	var preserved := not stable_after.is_empty() and after_chunks == before_chunks
	var no_partial := partial_after.is_empty()
	...
	ok = ok and failed_as_expected and preserved and no_partial and conservative_snapshot
	_emit({
		"ok": ok,
		...
		"previous_valid_state_preserved": preserved and no_partial,
