...
func _run() -> void:
	...
	var first: Variant = txn.call("import_file", store, ORIGINAL, {"imported_by": "alias_removal_probe"})
	var duplicate: Variant = txn.call("import_file", store, ALIAS, {"imported_by": "alias_removal_probe"})
	var before: Variant = store.call("search", MARKER, 8)
	var registry: Variant = KnowledgeSourceRegistryScript.new()
	var canonical_before: Variant = registry.call("record_for_source", ORIGINAL)
	var alias_before: Variant = registry.call("record_for_source", ALIAS)
	var removed: Variant = KnowledgeManagerScript.new().call("remove_source", ALIAS)
	var after_store: Variant = KnowledgeStoreScript.new()
	var after: Variant = after_store.call("search", MARKER, 8)
	var canonical_after: Variant = KnowledgeSourceRegistryScript.new().call("record_for_source", ORIGINAL)
	var alias_after: Variant = KnowledgeSourceRegistryScript.new().call("record_for_source", ALIAS)

	var duplicate_registered: bool = bool(duplicate.get("ok", false)) and bool(duplicate.get("duplicate", false)) and bool(duplicate.get("skipped", false))
	var canonical_survived: bool = not after.is_empty() and not canonical_after.is_empty()
	var alias_detached: bool = alias_after.is_empty()
	var ok: bool = bool(first.get("ok", false)) and duplicate_registered and not before.is_empty() and not canonical_before.is_empty() and not alias_before.is_empty()
