...
func _verify() -> void:
	...
	var store: Variant = KnowledgeStoreScript.new()
	var registry: Variant = KnowledgeSourceRegistryScript.new()
	var a_present: bool = not store.call("search", MARKER_A, 4).is_empty()
	var b_present: bool = not store.call("search", MARKER_B, 4).is_empty()
	var c_present: bool = not store.call("search", MARKER_C, 4).is_empty()
