...
# Runtime proof for two races that the generic benchmark did not cover:
...
# 2) search racing canonical source removal must not corrupt the store and the
#    committed post-remove state must remain readable.
...
func _search_remove_race() -> Dictionary:
	...
	if search_start != OK or remove_start != OK:
		# Release either waiter so a successfully started peer can terminate.
		search_ready.post()
		removal_done.post()
		if search_start == OK:
