...
func _ready() -> void:
	set_process(true)
	if OS.get_name() == "Windows": _start_backend_if_installed()
	_connect_ws()
...
func set_mode(mode: String, device: Variant = null) -> void:
	send_command("set_mode", {"mode": mode, "device": device})

func set_tts_state(playing: bool) -> void:
	send_command("tts_state", {"playing": playing})
