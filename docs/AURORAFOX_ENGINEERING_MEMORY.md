...
#### AF-MEM-085 — Git transport mirror отстаёт от GitHub API

- **Дата/среда:** 2026-09-24; Work; repository `Treninem/AI`; local HTTPS fetch reported main `4c6fe649af69c9be0eb080863f0e94b80cc3e082`, while authoritative GitHub branch API and PR base reported `394855e7c2a2439e159708573e46b1b6c6a5abb8`.
- **Симптом:** `git fetch origin main` завершался успешно, но remote-tracking ref оставался на старом SHA; PR оказался `mergeable_state=dirty` относительно фактического main.
- **Причина:** не подтверждена; наблюдается рассинхронизация доступного Git transport/mirror и GitHub API view.
- **Нерабочие попытки:** повторный успешный `git fetch` нельзя считать доказательством актуальности, если GitHub branch API показывает другой SHA.
- **Решение:** сравнивать SHA transport и authoritative branch API; при расхождении не менять release/main, интеграцию строить от API-confirmed tree/commit и сохранять оба SHA в журнале.
- **Профилактика:** preflight перед PR/merge/package: branch API SHA == локальный remote-tracking SHA; при mismatch блокировать обычный merge/push path.
- **Evidence:** draft PR #93, base `394855e7c2a2439e159708573e46b1b6c6a5abb8`; local mirror `4c6fe649af69c9be0eb080863f0e94b80cc3e082`.
- **Статус:** ENVIRONMENT.

#### AF-MEM-086 — master log feature-ветки стал бинарным blob

- **Дата/среда:** 2026-09-24; feature HEAD до исправления `deb25ada03a44ae5290fcaa895e43cdfa3860c52`.
- **Симптом:** `file` определял удалённый `docs/PROJECT_MASTER_LOG.md` как `data`; Git показывал 1,169 удалённых строк общей истории; чтение содержало бинарные/некорректные байты.
- **Причина:** не подтверждена; повреждение появилось в последовательности удалённых journal updates. Не приписывать конкретному connector/tool без дополнительного воспроизведения.
- **Нерабочие попытки:** открывать PR до UTF-8/diff проверки; обычный HTTPS push также недоступен без credentials и не исправляет blob.
- **Решение:** взять проверенную каноническую UTF-8 копию, проверить no-NUL/ключевые markers, заменить файл через contents API с текущим blob SHA; результат `7ad05479ab9b6c866bdb43f8291bff5e00117778`.
