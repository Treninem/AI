...
func _show_banner(main: Node) -> void:
	...
	var text := Label.new()
	text.text = "Чат уже доступен, но для ответов нужно один раз установить Ollama и базовые модели. Подготовка идёт отдельно и больше не перекрывает весь интерфейс."
