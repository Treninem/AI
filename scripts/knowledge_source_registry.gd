...
func inspect_file(path: String) -> Dictionary:
	...
	return {"ok": true, "path": path, "fingerprint_sha256": fingerprint, "size_bytes": size, "format": ext, "same_path_same_hash": same_path_same_hash, "duplicate": not by_hash.is_empty(), "duplicate_of": duplicate_of, "existing_source": by_path, "existing_role": existing_role, "matching_source": by_hash, "revision": maxi(1, existing_revision + (0 if same_path_same_hash else 1)), "source_id": _source_id(fingerprint)}
...
func register_duplicate(path: String, inspection: Dictionary) -> Dictionary:
	...
	if fingerprint.is_empty(): return {"ok": false, "error": "Нет fingerprint для регистрации дубля"}
	var rows := _load_rows()
	var index := _find_hash_index(rows, fingerprint)
	if index < 0: return {"ok": false, "error": "Исходный источник для дубля не найден"}
	var row: Dictionary = rows[index]
	var aliases: Array = row.get("aliases", []) if row.get("aliases", []) is Array else []
	if str(row.get("source", "")) != path and path not in aliases: aliases.append(path)
	row["aliases"] = aliases
	row["last_seen_at"] = Time.get_datetime_string_from_system(true)
	rows[index] = row
	if not _save_rows(rows): return {"ok": false, "error": "Не удалось сохранить alias источника"}
	return {"ok": true, "duplicate": true, "skipped": true, "source": row.get("source", path), "alias": path, "source_id": row.get("source_id", _source_id(fingerprint)), "fingerprint_sha256": fingerprint, "revision": row.get("revision", 1)}

func mark_imported(path: String, inspection: Dictionary, result: Dictionary, metadata: Dictionary = {}) -> Dictionary:
	var fingerprint := str(inspection.get("fingerprint_sha256", ""))
	if fingerprint.is_empty(): return {"ok": false, "error": "Нет fingerprint импортированного источника"}
	var rows := _load_rows()
	var existing_index := _find_path_index(rows, path)
	var matching_index := _find_hash_index(rows, fingerprint)
	var now := Time.get_datetime_string_from_system(true)
	var row := {"source_id": str(inspection.get("source_id", _source_id(fingerprint))), "source": path, "aliases": [], "fingerprint_sha256": fingerprint, "format": str(inspection.get("format", path.get_extension().to_lower())), "size_bytes": int(inspection.get("size_bytes", _file_size(path))), "revision": int(inspection.get("revision", 1)), "chunks": int(result.get("chunks", 0)), "structured_records": int(result.get("records", result.get("structured_records", 0))), "kinds": result.get("routed", {}), "parser": str(metadata.get("imported_by", "knowledge_store")), "parser_version": str(metadata.get("parser_version", "1")), "status": "ready", "imported_at": now, "updated_at": now, "last_seen_at": now}
	if existing_index >= 0:
		...
		if old_hash == fingerprint:
			...
			if matching_index >= 0:
				...
				if path != str(canonical.get("source", "")) and path not in canonical_aliases: canonical_aliases.append(path)
		...
		if path != str(canonical.get("source", "")) and path not in aliases: aliases.append(path)
		canonical["aliases"] = aliases
		canonical["last_seen_at"] = now
		rows[matching_index] = canonical
		row = canonical
	else: rows.append(row)
	if not _save_rows(rows): return {"ok": false, "error": "Не удалось сохранить реестр источников"}
...
func remove_source(source: String) -> Dictionary:
	...
	return {"ok": ok, "source": source, "removed": removed, "aliases_removed": aliases_removed, "canonical_removed": canonical_removed}

func sources() -> Array: return _load_rows().duplicate(true)
...
func storage_is_valid() -> bool:
	if not FileAccess.file_exists(REGISTRY_PATH): return false
	_load_rows()
	return _cache_valid
...
func stats() -> Dictionary:
	...
	for value in rows:
		...
		if row_aliases is Array: aliases += row_aliases.size()
		revisions += maxi(1, int(row.get("revision", 1)))
	return {"sources": rows.size(), "aliases": aliases, "revisions_total": revisions, "bytes": _file_size(REGISTRY_PATH), "path": REGISTRY_PATH}
...
func _find_path_index(rows: Array, path: String) -> int:
	for i in range(rows.size()):
		...
		if str(row.get("source", "")) == path: return i
		var aliases = row.get("aliases", [])
		if aliases is Array and path in aliases: return i
	return -1
...
func _find_hash_index(rows: Array, fingerprint: String) -> int:
	for i in range(rows.size()):
		if rows[i] is Dictionary and str(rows[i].get("fingerprint_sha256", "")) == fingerprint: return i
	return -1
func _source_id(fingerprint: String) -> String: return "src_" + fingerprint.substr(0, mini(24, fingerprint.length()))
...
func _load_rows() -> Array:
	...
	if _cache_valid and _cache_generation == _registry_write_generation and size == _cache_size and mtime == _cache_mtime: return _cached_rows.duplicate(true)
	var file := FileAccess.open(REGISTRY_PATH, FileAccess.READ)
	if file == null:
		_cache_valid = false; return []
	var parsed = JSON.parse_string(file.get_as_text()); file.close()
	if not parsed is Dictionary or int(parsed.get("schema_version", 0)) != REGISTRY_VERSION:
		_cache_valid = false; return []
...
func _remember_cache(rows: Array, size: int, mtime: int) -> void:
	_cached_rows = rows.duplicate(true); _cache_size = size; _cache_mtime = mtime; _cache_generation = _registry_write_generation; _cache_valid = true
func _save_rows(rows: Array) -> bool:
	...
	file.store_string(JSON.stringify({"schema_version": REGISTRY_VERSION, "updated_at": Time.get_datetime_string_from_system(true), "sources": rows}, "  ")); file.close()
	var target_abs := ProjectSettings.globalize_path(REGISTRY_PATH); var temp_abs := ProjectSettings.globalize_path(temp)
	if FileAccess.file_exists(REGISTRY_PATH):
		var remove_error := DirAccess.remove_absolute(target_abs)
		if remove_error != OK:
			DirAccess.remove_absolute(temp_abs); _cache_valid = false; return false
	var renamed := DirAccess.rename_absolute(temp_abs, target_abs) == OK
	if not renamed:
		_cache_valid = false; return false
	_registry_write_generation += 1
