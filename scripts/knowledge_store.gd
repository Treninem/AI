...
func import_text(text: String, source := "manual", metadata: Dictionary = {}) -> Dictionary:
	...
	_ensure_dir(); var chunks := _chunk(clean)
	var kind := str(metadata.get("kind", _classify_record(source, clean)))
	for chunk in chunks:
		var meta := metadata.duplicate(true); meta["kind"] = kind
		_append(DB_PATH, {"id": _id(source, chunk), "kind": kind, "source": source, "text": chunk, "metadata": meta, "created_at": Time.get_datetime_string_from_system(true)})
	return {"ok": true, "source": source, "chunks": chunks.size(), "kind": kind, "path": DB_PATH}
...
func import_file(path: String, metadata: Dictionary = {}) -> Dictionary:
	...
	if str(extracted.get("mode", "")) == "json": return import_json_file(path, metadata)
	var meta := metadata.duplicate(true)
	meta["format"] = path.get_extension().to_lower(); meta["original_file"] = path
...
func import_json_file(path: String, metadata: Dictionary = {}) -> Dictionary:
	...
	var raw := file.get_as_text(); file.close(); var parsed = JSON.parse_string(raw)
	if parsed == null: return {"ok": false, "error": "Некорректный JSON", "path": path}
	var records: Array = []; _flatten_json(parsed, "$", records)
	var routed := {"algorithm": 0, "template": 0, "skill": 0, "fact": 0, "example": 0, "instruction": 0, "knowledge": 0}; var seen := {}
	for row in records:
		var text := _record_text(row.get("value")); if text.strip_edges().is_empty(): continue
		var fingerprint := _id(path + str(row.get("json_path", "")), text); if seen.has(fingerprint): continue
		seen[fingerprint] = true; var kind := _classify_record(str(row.get("json_path", "")), row.get("value")); routed[kind] = int(routed.get(kind, 0)) + 1
		var meta := metadata.duplicate(true); meta.merge({"kind": kind, "format": "json", "json_path": row.get("json_path", "$"), "original_file": path}, true)
...
func search(query: String, limit := 6) -> Array:
	...
	scored.sort_custom(func(a, b): return int(a.score) > int(b.score)); var out: Array = []
...
func _chunk(text: String) -> Array[String]:
	...
	for paragraph in paragraphs:
		var p := str(paragraph).strip_edges(); if p.is_empty(): continue
...
func _ensure_dir() -> void: DirAccess.make_dir_recursive_absolute(ProjectSettings.globalize_path("user://knowledge"))
func _id(source: String, text: String) -> String: return "%s-%s" % [str(source.hash()).replace("-", "n"), str(text.hash()).replace("-", "n")]
