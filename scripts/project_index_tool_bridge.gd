...
func _register_tools() -> void:
	...
	registry.register_tool(
		"index_project",
		"Проиндексировать большой проект локально: исходники, языки, классы, функции и текст. Повторный запуск обновляет только изменившиеся файлы.",
...
func _index_project(args: Dictionary) -> Dictionary:
	...
	if not _allowed(path): return {"ok": false, "error": "Index path denied. Use res:// or user://."}
...
func _search_project(args: Dictionary) -> Dictionary:
	var path := str(args.get("path", "res://"))
	if not _allowed(path): return {"ok": false, "error": "Search path denied. Use res:// or user://."}
...
func _search_symbols(args: Dictionary) -> Dictionary:
	var path := str(args.get("path", "res://"))
	if not _allowed(path): return {"ok": false, "error": "Symbol path denied. Use res:// or user://."}
	var query := str(args.get("query", "")).strip_edges()
