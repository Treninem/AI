class_name TrustedProjectSandboxBridge
extends Node
...
func _register() -> void:
	...
	registry.register_tool(
		"trusted_project_read",
		"Прочитать конкретный текстовый файл из доверенного проекта без предоставления доступа ко всему диску.",
		{"project_path":"string","relative_path":"string","max_chars":"int"},
		Callable(self, "_read_project_file")
	)
	registry.register_tool(
		"project_compare_file",
		"Сравнить файл из активной песочницы с оригиналом доверенного проекта и вернуть SHA-256/размер/текстовый diff-подобный фрагмент.",
		{"project_path":"string","relative_path":"string","sandbox_path":"string"},
		Callable(self, "_compare_file")
	)
	registry.register_tool(
		"project_apply_file",
		"Применить один проверенный файл из активной песочницы обратно в доверенный проект. Перед заменой всегда создаёт локальную резервную копию оригинала.",
		{"project_path":"string","relative_path":"string","sandbox_path":"string"},
		Callable(self, "_apply_file")
	)
	_registered = true
...
func _workspace_root() -> String:
	...
	return ProjectSettings.globalize_path("user://sandboxes/%s" % id).replace("\\", "/")
...
func _import_project(args: Dictionary) -> Dictionary:
	...
	var ws_root := _workspace_root()
	if ws_root.is_empty(): return {"ok": false, "error": "Create a workspace first"}
...
func _sandbox_file(relative: String) -> String:
	...
	if ws_root.is_empty() or rel.is_empty(): return ""
	return "%s/work/%s" % [ws_root, rel]
...
func _compare_file(args: Dictionary) -> Dictionary:
	...
	var candidate := _sandbox_file(sandbox_rel)
	...
	var result := {
		"ok": true,
		"changed": original_hash != candidate_hash,
		"original_exists": FileAccess.file_exists(original),
		"original_sha256": original_hash,
		"candidate_sha256": candidate_hash,
		"original_size": _file_size(original),
		"candidate_size": _file_size(candidate)
	}
	if _looks_text(rel) and _file_size(candidate) <= 2 * 1024 * 1024:
		result["preview"] = _text_change_preview(original, candidate)
	return result
...
func _apply_file(args: Dictionary) -> Dictionary:
	...
	var sandbox_rel := _safe_relative(str(args.get("sandbox_path", rel)))
	if rel.is_empty() or sandbox_rel.is_empty(): return {"ok": false, "error": "Invalid path"}
	var source := _sandbox_file(sandbox_rel)
	...
	if not before.is_empty() and before == candidate:
		return {"ok": true, "changed": false, "path": target, "sha256": candidate}
