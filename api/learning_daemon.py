...
def main() -> int:
    ...
    bridge = AuroraRuntimeBridge(
        ...
        timeout=max(1, min(20, float(os.getenv("AURORAFOX_BRIDGE_TIMEOUT", "20")))),
    )
    sync = LearningSynchronizer(root, bridge)
    batch_size = max(1, min(1000, int(os.getenv("AURORAFOX_LEARNING_BATCH", "250"))))
    result = sync.flush(batch_size, max_seconds=60)
    LOG.info(
    ...
    # A failed oneshot is retried by the timer too. Do not hide delivery failure.
